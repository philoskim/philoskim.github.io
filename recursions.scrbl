...
재귀 (tail recursion)라고 부른다(단, 모든 종류의 재귀 함수를 꼬리 재귀 함수의 형태로 바꿀
수 있는 것은 아니다). 따라서 꼬리 호출 최적화 기법을 적용할 수 있는 조건에 맞게
프로그래머가 재귀 함수를 작성하면, 컴파일러가 이 함수를, stack overflow를 일으키지 않는
함수로 자동으로 변환해 준다. 이 과정을 TCO(Tail Call Optimization: 꼬리 호출 최적화)라고
부른다.
...
그런데 위에서 정의한 @tt{fact-1} 함수는, 꼬리 재귀 함수가 아니다. 흔히들 꼬리 부위라고
하면, 위의 코드 중에, (3) 또는 (4) 부분을 꼬리라고 생각하기 쉬운데, 함수의 말단 부분에
위치해 있다고 해서 그 부분을 무조건 꼬리라고 하지는 않는다는 점에 주의해야 한다. 꼬리
재귀에서, '꼬리'의 가장 정확한 개념은 '함수가 최종적으로 리턴값을 반환하는 곳'을
...
된다. 따라서 위의 코드는 꼬리 재귀함수의 형태가 아니다. 그래서 10000과 같은 큰 수를 인수로
주고 함수를 실행하니, StackOverflowError가 발생하였다.
...
@itemlist[
  ...
  @item{(2)에서 @tt{fact-2}의 인수가 하나이므로, 라인 2의 함수를 호출한다. 그런데
...
위의 코드에서는, (1)로 표시된 꼬리의 위치에 @tt{fact-2} 함수가 다시 위치해 있으므로, 이
함수는 꼬리 재귀 함수의 조건을 충족시킨 코드이다. 꼬리 재귀 함수 이해의 핵심은, 다음
연산에 필요한 모든 상태(state) 값을 '함수의 인수 형태'로 모두 건네주는 데 있다. 위의
코드에서는, 재귀 호출 때마다의 계산 결과(상태 값 @tt{n}과 @tt{acc}) 값을, @tt{fact-2}의
인수로 다시 전달하고 있음에 주목해야 한다.

그런데 (3)의 실행 결과를 보면, 여전히 StackOverflowError가 발생하고 있다. 만약 위의 코드가
Common Lisp이나 Scheme 언어로 작성되었다면, 이 언어들의 컴파일러들은 (1)의 부분이 TCO를
적용할 수 있는 부분임을 알아채고, 이 함수를 stack을 소모하지 않는 함수로 '자동으로'
변환하는 작업을 수행해 주므로, StackOverflowError가 발생하지 않는다. 하지만 Clojure에서는
프로그래머가 '수동으로' 꼬리 재귀를 하는 부분이라는 것을 컴파일러에게
'명시적으로' 알려 주어야만 하는데, special-form인 @tt{recur}가 이 역할을 수행한다.
...
기반하고 있는 Java의 컴파일러는 자체적으로 이 TCO 기능을 지원하지 않았다. 그래서 이 기능은
Clojure 컴파일러가 별도로 지원할 수 밖에 없었는데, Clojure 언어의 창시자인 Rich
Hickey는, 프로그래머가 special-form인 @tt{recur}를 통해 직접 '수동으로' Tail Recursion을
지정하는 방식을 택했다. 이로부터 얻는 이점은, 코드를 분석할 때 @tt{recur} 호출이 있으면,
꼬리 재귀 함수임을 곧바로 파악할 수 있다는 데 있다. 반면에, Common Lisp이나 Scheme의
경우에는, 꼬리 재귀 함수 최적화 기법이 실제로 적용되는 경우인지 아닌지를 파악하려면,
...
(3)과 (4)처럼 익명 함수(anonymous function)의 형태로 변경해 준 후에, @tt{trampoline}
함수를 통해 호출해 주면 된다. 다시 말해, @tt{trampoline} 함수는 mutual tail recursion
호출을 구현하기 위해 Clojure에 도입된 함수다.

여기서 가장 중요한 점은, 위의 (1)의 경우에 @tt{(my-odd?-1 ...)} 코드는, 이미 정의되어 있는
기존의 함수 @tt{my-odd?-1}을, 단지 반복해서 호출하는 것임에 반해, 아래의 (3)의
@tt{#(my-odd?-2 ...)} 코드는, 실행될 때마다 매번 '새로 생성'된 익명 함수 객체를 리턴값으로
반환하는 데 있다. 리습 계열 언어에서는 이와같이 코드 실행 중에 새로운 함수를 동적으로
생성할 수 있는데, 이 차이점을 정확하게 이해하지 못하면, 아래의 코드를 이해하는데 지장이
있을 것이다.

이 새로 생성된 함수 객체 안에, 이전 단계에서 처리한 state(상태 값)를 저장한 후에, 이
객체를 '다음 단계에 실행될 함수'의 '인수' 형태로 넘기게 되므로, stack을 사용하지 않게
만든다.
...
'끝낸 후', 다시 새로이 리턴받은 함수(여기서는 ret)를 호출하는 기법을 써서 스택 쌓이는 것을
