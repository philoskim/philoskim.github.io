...
다음의 경우에서처럼, 쓰레드 C의 처리 결과에 따라 쓰레드 F로 곹바로 점프하는 것도
가능합니다.
...
다음의 경우에서처럼, 쓰레드 C의 처리 결과에 따라 다른 파이프 라인을 타는 것도 가능합니다.
...
다음의 경우에서처럼, 쓰레드 D의 처리 결과에 따라 루프를 타는 것도 가능합니다.
...
위에 든 예들 중에서, 2번의 경우를 core.async로 구현한 예는 다음과 같다.
...
@descblock|{
(chan): 채널을 하나 생성한다.
(>!! ch 1): 채널 ch에 1이라는 값을 집어 넣는다.
(<!! ch): 채널 ch에 담겨 있는 값 한 개를 빼낸다. 채널에 아무런 값도 없을 때에는
          무한정 기다린다(이로 인해 동기적인 처리가 가능해딘다).
}|

위의 예에서 볼 수 있듯이, core.async에서는 channel이라는 중계 수단을 거쳐 비동기적인
쓰레드들간의 동기화를 이훈다. 코드가 대단히 직관적이고 단순하다. 클로저 프로그램밍의
핵심적인 가치인 '단순함'의 추구의 결과물이다. 무엇보다도 코드는 구조가 단순해야
한다. 그래야 프로그래머가 버그를 양산하지 않게 된다. 이것을 자바스크립트로 구현하려면
콜백함수 안에 콜백함수를 여러 번 중첩해야만 해서 대단히 복잡한 구조의 코딩을 해야만
...
Clojure의 core.async에 있는 함수들은 크게 두 종류로 나쥔다. API 문서를 보시면 알겠지만,
