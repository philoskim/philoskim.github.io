...
라인 수도 거의 같다. 그러면 이 두 프로그램은 등가인가? 저는 그렇지 않다고
본다. 파이썬은 객체 지향 언어이어서, 프로그래밍 단위가 클래스일 수밖에 없다는 점에서,
프로그래밍 단위가 함수 단위인 클로저 코드에 비해서 복잡성은 훨씬 큰 것이다. 복잡성이라는
기준에서 볼 때는 큰 차이가 있는 것이고, 아마도 클래스의 크기가 늘어날 수록 그 복잡성은
기하급수적으로 늘어날 수 있다는 점에서, Clojure로 작성된 코드가 더 우월하다고 나는
판단한다.
...
비교를 통해 함수형 프로그램밍이 무엇인가애 대한 이야기를 풀어 나가겠다. 많은 사람들이
Common Lisp을 함수형 프로그래밍 언어로 알고 있는 것 같다. 이런 견해는 함수형 언어를 어떻게
정의하느냐에 따라, 맞는 견해일 수도 있고, 부적절한 표현일 수도 있다. 이번에는 내가
이해하고 있는 함수형 언어에 대해 이야기해 보겠다.
...
하지만, 이것을 오히려 예외적인 상황으로 간주된다.  그렇게 되면 디버깅할 때 디버깅의 단위가
함수 하나로 국한된다. 객체 지향 프로그래밍에서처럼, 클래스 내의 다른 멤버 함수가 클래스의
내부 변수를 언제 어떻게 건드리는지 전혀 신경쓸 필요가 없게 된다.
