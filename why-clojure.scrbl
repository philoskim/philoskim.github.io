...
보고 있기 때문에, 나는 함수형 언어를 특히 리습 계열 언어인 Clojure를 높게 평가한다.
...
바이트 코드로 직접 컴파일한다. 그리고 병목을 일으키는 부분을 profiling을 실행해 찾아낸 후,
type hinting 정보를 추가해 주면 실행 속도를 향상시킬 수도 있다. 참고로, 자바 코드를 직접
호출하는 부분은 자바와 실힝 속도가 동일하다.
...
함수 하나만 변경하면 된다. 결과적으로 프로그램의 '구조'가 간단하기 떄문에 가능한 일이다.
