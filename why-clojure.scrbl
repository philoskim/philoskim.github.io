...
비교를 통해 '함수형 프로그래밍이란 무엇인가'에 대한 이야기를 풀어 나가겠다. 많은 사람들이
...
등장하는 언어들 거의 대부분이 위의 네 조건을 모두 충족시키고 있다는 것이다(결론적으로,
리습을 흉내내고 있다고 봐도 무방하다). JavaScript마저도 위의 네 조건을 모두 충족시키고
있는데, 그렇다면 JavaScript도 함수형 언어라고 불러야 한다.
...
진정한 의미의 함수형 언이라고 할 수 있다. 반면에 Common Lisp은 엄밀한 의미에서의 함수형
언어는 아니다.
...
같은 조건을 충족하는 언어로 Haskell 같은 함수형 언어도 있기는 하다. 하지만 Haskell은
학문적 관점에서 출발한 언어여서 더 엄밀한 맛은 있지만, 프로그래머의 관점에서 볼 때
실용성은 클로저에 비해 현저히 떨어진다. 클로저의 창시자인 Rich Hickey 자신이 수십년간
대규모 프로젝트에서 활동해 온 실전 프로그래머였다는 점에서, 클로저는 소규모의 프로젝트를
수행하는 대학 연구실의 상황과는 다른 상황을 위해 설계된 언어이다.
...
이론도 아인쉬타인 자신에게는 '쉬운' 것이다. 흔히 드는 예이지만, 9개의 공을 저글링 하는
...
쉽다. 하라는 대로 하다 보면 금방 결과물이 나온다. 하지만, 기존에 쓰여 있는 코드를 수정해야
한다고 해 보자. 실제로 이런 일은 비일비재하다. 그때부터는 무지하게 어려워진다. 무엇 하나
고치려 해도 클래스 전체의 구조를 알아야 하는데, 하나의 멤버 변수를 이 메소드 저 메소드가
건드리고 있고(즉, 메소드 간의 상호 의존성이 심하다), 또 멤버 변수들 간의 관계도 파악해야
한다. 게다가 남이 쓴 코드를 이해하는 것이 어디 쉬운 일인가? 한 마디로, 조금만 깊이
들어가게 되면 그때부터는 급격히 어려워 진다. 왜 그런가? 프로그램의 구조가 복잡하기
때문이다. 객체 지향 프로그램이 절차 지향 프로그램보다 우월하다고는 하지만, 역시
상태(state)를 변경하면서 작업한다는 점에서는 오십보 백보이기 때문이다. 클로저에서는 이런
일이 상대적으로 더 쉽다. immutable 데이터를 다루기 때문에 함수들 간에 의존성이 없어서,
자신이 기존의 소스에서 무엇인가 변경하고자 할 때, 대개의 경우 함수 하나만 변경하면
된다. 결과적으로 프로그램의 '구조'가 간단하기 떄문에 가능한 일이다.
