...
함수는 꼬리 재귀 함수의 조건을 충족시킨 코드이다. 꼬리 재귀 함수 이해의 핵심은, 계산 중에
발생하는 모든 상태(state) 값을 '함수의 인수 형태'로 건네줌으로써, 재귀 함수 내부에 어떠한
상태(state) 값도 지니지 않게 하는 데 있다. 위의 코드에서는, 재귀 호출 때마다의 계산
결과(상태 값 @tt{n}과 @tt{acc}) 값을, @tt{fact-2}의 인수로 다시 전달하고 있음에 주목해야
한다.
